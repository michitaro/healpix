...
window.addEventListener('load', function (e) {
    ...
    overlayCanvas.onmousemove(function (phi, theta) {
        ...
        healpix.query_disc_inclusive_nest(nside, v, 0.1, function (ipix) {
...
function pixelPath(canvas, nside, ipix, nstep) {
    ...
    for (var i = 0; i < nstep; ++i) {
        var ne = i / nstep;
        var _a = rotateZ(healpix.pixcoord2vec_nest(nside, ipix, ne, 0), v0), x = _a[0], y = _a[1], z = _a[2];
        var phi = Math.atan2(y, x) + phi0;
        lineTo(phi, Math.acos(z));
    }
    for (var i = 0; i < nstep; ++i) {
        var nw = i / nstep;
        var _b = rotateZ(healpix.pixcoord2vec_nest(nside, ipix, 1, nw), v0), x = _b[0], y = _b[1], z = _b[2];
        var phi = Math.atan2(y, x) + phi0;
        lineTo(phi, Math.acos(z));
    }
    for (var i = 0; i < nstep; ++i) {
        var ne = 1 - i / nstep;
        var _c = rotateZ(healpix.pixcoord2vec_nest(nside, ipix, ne, 1), v0), x = _c[0], y = _c[1], z = _c[2];
        var phi = Math.atan2(y, x) + phi0;
        lineTo(phi, Math.acos(z));
    }
    for (var i = 0; i < nstep; ++i) {
        var nw = 1 - i / nstep;
        var _d = rotateZ(healpix.pixcoord2vec_nest(nside, ipix, 0, nw), v0), x = _d[0], y = _d[1], z = _d[2];
        var phi = Math.atan2(y, x) + phi0;
        lineTo(phi, Math.acos(z));
    }
